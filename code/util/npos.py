#copied from https://github.com/w3f/consensus/blob/master/NPoS/npos.py
import unittest
...
def find_id(candidates):
    id_set = set([can.validator_id for can in candidates])
    pot_set = set(range(len(candidates)+1))
    diff_set = pot_set.difference(id_set)
...
def phragmms(votelist, num_to_elect, tolerance=0.1, papp=False):
    nomlist, candidates = setuplists(votelist)
    calculate_approval(nomlist)
    ...
    for round in range(num_to_elect):
        ...
        elected_candidate.electedscore = score
        
        if papp:
            #create new candidate
            new_val_id = find_id(candidates)
            newcandidate = candidate(new_val_id, len(candidates))
            newcandidate.duplicate = True 
            if elected_candidate.duplicate: 
                newcandidate.copyOf = elected_candidate.copyOf
            else:
                newcandidate.copyOf = elected_candidate.validator_id
            
            
            #add this edge to all of the supporters of this candidate
            for nom in nomlist:
                for ed in nom.edges: 
                    if ed.validator_id == elected_candidate.validator_id: 
                        new_edge = edge(nom.nominator_id,new_val_id)
                        new_edge.candidate = newcandidate
                        nom.edges += [new_edge]
                        newcandidate.approval_stake += nom.budget
            
            assert newcandidate.approval_stake == elected_candidate.approval_stake
            
            #add this candidate to the candidate list
